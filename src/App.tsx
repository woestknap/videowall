...
function SceneEditorPage({ sceneId }: { sceneId: string }) {
  ...
  return <main className="editor-page">
    ...
    <div className="editor-layout"><aside className="editor-toolbar"><div className="screen-list"><p className="eyebrow">SCREENS IN THIS SCENE</p><small>Tick the displays used by this scene. Their real proportions stay intact on the map.</small>{devices.map((item) => <div className="screen-list-item" key={item.id}><label><input type="checkbox" checked={isSceneDevice(item.id)} onChange={() => toggleSceneDevice(item.id)} /> <span>{item.name}</span></label><div><label>W<input type="number" min="1" value={item.layout_width ?? 16} onChange={(event) => updateDeviceLayout(item.id, { layout_width: Math.max(1, Number(event.target.value)) })} /></label><label>H<input type="number" min="1" value={item.layout_height ?? 9} onChange={(event) => updateDeviceLayout(item.id, { layout_height: Math.max(1, Number(event.target.value)) })} /></label></div></div>)}</div><p className="eyebrow">ADD A LAYER</p><button onClick={() => addLayer('text')}>T Text</button><button onClick={() => addLayer('clock')}>◷ Clock</button><button onClick={() => addLayer('ticker')}>↔ Ticker</button><button onClick={() => addLayer('image')}>▣ Image</button><button onClick={() => addLayer('video')}>▶ Video</button><small>Drag a screen label on the map to match the real wall. Drag a layer to position it.</small></aside>
