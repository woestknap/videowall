...
function SceneEditorPage({ sceneId }: { sceneId: string }) {
  ...
  const [guides, setGuides] = useState<{ stage: GuideBox | null; devices: GuideBox[]; layers: GuideBox[] }>({ stage: null, devices: [], layers: [] })
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      const toGuideBox = (box: DOMRect): GuideBox => ({ left: box.left - wrapBox.left, top: box.top - wrapBox.top, width: box.width, height: box.height })
      setGuides({
        stage: toGuideBox(stage.getBoundingClientRect()),
        devices: [...stage.querySelectorAll<HTMLElement>('.device-mask')].map(element => toGuideBox(element.getBoundingClientRect())),
        layers: [...stage.querySelectorAll<HTMLElement>('.canvas-layer.selected-layer')].map(element => toGuideBox(element.getBoundingClientRect())),
      })
    }
  ...
  }, [zoom, pan.x, pan.y, scene, devices, selectedId])
