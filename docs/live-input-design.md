...
The live-source foundation exists, but wall transport does not. `SceneLayer.type` can be `live`, its JSON content can reference a `liveSourceId`, and the editor can enumerate a local camera, capture video-only media and show that stream in the editor. A TypeScript `LiveSource` shape exists, but there is no separate live-source table. The captured `MediaStream` remains browser-local. `Layer` deliberately renders live layers as `null`, and the player has no WebRTC or signalling path.

This document designs LIVE-05. It does not make live video playable, add a backend service, create database objects or alter scene data. A live-source definition and a live session are different things: scene JSON may identify the source, but it must never contain a `MediaStream`, SDP, ICE candidates, peer connections, player credentials or session secrets.
