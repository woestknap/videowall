...
- **Transient player media disconnect:** keep both ends of the peer and the mapped `MediaStream` for a five-second grace period while exposing the browser's real WebRTC and ICE states. Cancel teardown if the peer reconnects; otherwise close it and request a fresh negotiation. Re-authenticating a signalling socket for the same usable media session does not send `peer-ready`, create a new offer, discard the peer or blank its live layer.
...
Production replaces temporary quick-tunnel hostnames with a stable WSS hostname backed by a Cloudflare named tunnel. The tunnel carries signaling only and does not relay WebRTC media or weaken the service's authentication. See [production signaling deployment](production-signaling.md) for Pages variables, server-only configuration, startup order and troubleshooting.
